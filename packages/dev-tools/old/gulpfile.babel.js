...
const loadReposFromCache = () => fs.readFile.promise(PROJECTS_CACHE, 'utf8').then(JSON.parse);
...
const git = (cmd, cwd) =>
  exec.promise(`git ${cmd}`, { cwd }).catch((err) => {
...
const linkNodeModules = R.curry((repo) =>
  exec
    .promise('ln -s ../../node_modules .', { cwd: pathToRepo(repo) })
    .catch(() => console.log(`${repo.name}: already has node_modules`))
);

const linkPackage = R.curry((repo) =>
  exec
    .promise(`ln -s ${pathToRepo(repo)} ${__dirname}/node_modules`)
...
gulp.task('link-packages', ['clone-repos'], () =>
  loadRepos().then(R.map(linkPackage)).then(Promise.all)
);
