...
const sortKeys = (obj) => {
  ...
  Object.keys(obj)
    .sort()
    .forEach((key) => (results[key] = obj[key]));
  return results;
};
...
find('../brush-!(base)/package.json').then((files) =>
  Promise.all(
    files.map((file) =>
      fs.promise
        .readFile(`${path.dirname(file.fullpath)}/README.md`, 'utf8')
...
find('../brush-!(base)/babel.js').then((files) =>
  Promise.all(files.map((file) => fs.promise.unlink(file.fullpath)))
);

find('../brush-!(base)/mocha.opts').then((files) =>
  Promise.all(
    files.map((file) =>
      Promise.resolve()
